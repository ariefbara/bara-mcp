...
class MissionCommentControllerTest extends AsProgramConsultantTestCase
{
    ...
    protected function executeSubmit()
    {
        ...
        $uri = $this->asProgramConsultantUri . "/missions/{$this->missionOne->id}/mission-comments";
        $this->post($uri, $this->submitRequest, $this->consultant->personnel->token);
    }
    public function test_submit_201()
    {
        ...
        $response = [
            ...
            'rolePaths' => json_encode([
                'mentor' => $this->consultant->id
            ]),
            'userName' => $this->consultant->personnel->getFullName(),
        ...
        $missionCommentEntry = [
            ...
            'message' => $this->submitRequest['message'],
            'userName' => $this->consultant->personnel->getFullName(),
            'userId' => $this->consultant->personnel->id,
            'modifiedTime' => (new DateTimeImmutable())->format('Y-m-d H:i:s'),
    ...
    protected function executeReply()
    {
        ...
        $uri = $this->asProgramConsultantUri . "/mission-comments/{$this->missionCommentOne->id}";
        $this->post($uri, $this->submitRequest, $this->consultant->personnel->token);
    }
