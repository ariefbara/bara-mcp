...
class MentoringListFilterForCoordinator
{

    const MENTORING_REQUEST_TYPE = 'mentoring-request';
    const MENTORING_SLOT_TYPE = 'mentoring-slot';
    const BOOKED_MENTORING_SLOT_TYPE = 'booked-mentoring-slot';
    ...
    public function addType(?string $type)
    {
        if (in_array($type, [
            ...
            self::BOOKED_MENTORING_SLOT_TYPE, 
        ...
        foreach (array_unique($this->typeList) as $type) {
            switch ($type) {
                ...
                case self::MENTORING_SLOT_TYPE:
                    ...
                    break;
                case self::BOOKED_MENTORING_SLOT_TYPE:
                    $typeSelection .= empty($typeSelection) ? "_mentoring.bookedMentoringSlotId IS NOT NULL": "OR _mentoring.bookedMentoringSlotId IS NOT NULL";
                    break;
...
_STATEMENT;
    ...
    AND (
        ...
        OR _mentoring.bookedMentoringSlotId IS NOT NULL
    )
_STATEMENT;
    ...
    AND (
        ...
        OR _mentoring.totalBooking IS NULL
    )
_STATEMENT;
            default:
                return null;
